...
def read_MODIS_points_to_domain(modis_path,min_x,max_x,min_y,max_y):
    ...
    for file_name in file_names:
        ...
        points = reproject_points(points, 4326, 3413)
        ...
        indices_x = np.logical_and(points[:,0]>=min_x-resolution_buffer,
                                     points[:,0]<=max_x+resolution_buffer)
        indices_y = np.logical_and(points[:,1]>=min_y-resolution_buffer,
                                     points[:,1]<=max_y+resolution_buffer)
...
def interpolate_points_to_grid(points, X, Y, band_1_points, band_3_points, band_4_points):
    ...
    band_1_grid = griddata(points,band_1_points.ravel(),(X,Y), method='nearest', fill_value=0)
    band_1_grid[band_1_grid<0]=0
    print(np.max(band_1_grid))
    # C = plt.imshow(band_1_grid)
    # plt.colorbar(C)
    # plt.show()
    print('    - Interpolating band 3')
    band_3_grid = griddata(points,band_3_points.ravel(),(X,Y), method='nearest', fill_value=0)
    band_3_grid[band_3_grid < 0] = 0
    print('    - Interpolating band 4')
    band_4_grid = griddata(points,band_4_points.ravel(),(X,Y), method='nearest', fill_value=0)
    band_4_grid[band_4_grid < 0] = 0
...
config_dir = '/Volumes/petermann/Research/Projects/Ocean_Modeling/Projects/Downscale_Greenland/' \
...
points = reproject_points(np.column_stack([Lon.ravel(), Lat.ravel()]), 4326, 3413)
...
points, band_1_points, band_3_points, band_4_points = \
    read_MODIS_points_to_domain(modis_path,min_x,max_x,min_y,max_y)
...
x = np.arange(np.shape(X)[1])
y = np.arange(np.shape(X)[0])
...
# grid = np.concatenate([np.reshape(band_1_grid,(np.shape(band_1_grid)[0], np.shape(band_1_grid)[1], 1)),
#                        np.reshape(band_3_grid,(np.shape(band_3_grid)[0], np.shape(band_3_grid)[1], 1)),
#                        np.reshape(band_4_grid,(np.shape(band_4_grid)[0], np.shape(band_4_grid)[1], 1))], axis=2)
# plt.imshow(grid)
# plt.show()
