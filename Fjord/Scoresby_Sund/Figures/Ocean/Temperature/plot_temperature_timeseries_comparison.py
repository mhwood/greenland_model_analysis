...
def create_timeseries_plot(output_file, plot_titles, ecco_shelf_timeseries,
                           ...
                           omg_temperature_timeseries, omg_locations, for_publication=False):
    ...
    ax1.text(1992.2, np.max(regional_ecco_timeseries[~np.isnan(regional_ecco_timeseries[:,1]),1]),
             'a) Regional Ocean', ha='left', va='top', fontsize=12)
    ...
    ax2.text(1992.2, np.max(ecco_shelf_timeseries[:, 1]), 'b) Sound Entrance', ha='left', va='top', fontsize=12)
    ...
    ax3.legend(loc=9,ncols=2)
    ax3.text(1992.2, np.max(L2_temperature_timeseries[L2_locations.index('Mid-Sound')][:, 1]), 'c) Mid-Sound', ha='left', va='top', fontsize=12)
    ...
    ax4.legend(loc=9)
    ax4.text(1992.2, np.max(L2_temperature_timeseries[L2_locations.index('Near-DJG')][:, 1]),
             'd) Near-DJG', ha='left', va='top', fontsize=12)

    if for_publication:
        plt.savefig(output_file[:-3]+'pdf',dpi=300)
    else:
...
create_timeseries_plot(output_file, plot_titles, ecco_shelf_timeseries,
                       ...
                       L1_time, L1_temperature_timeseries, L1_locations, L1_RMSE,
                       L2_time, L2_temperature_timeseries, L2_locations, L2_RMSE,
                       omg_temperature_timeseries, OMG_locations, for_publication=True)