...
def read_nc_profile(project_folder, model_name, var_name):

    timeseries_file = os.path.join(project_folder, 'Data', 'Ocean', 'L1', model_name+'_'+var_name+'_timeseries.nc')
...
def create_profile_figure(project_folder, var_name, time, depth, var_grid):
    ...
    output_file = os.path.join(project_folder,'Figures','L1_Disko_Bay_'+var_name+'_Profile_Timeseries.png')
...
time, depth, var_grid = read_nc_profile(project_folder, model_name, var_name)

create_profile_figure(project_folder, var_name, time, depth, var_grid)
