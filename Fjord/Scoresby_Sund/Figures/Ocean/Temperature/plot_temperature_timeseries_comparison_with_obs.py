...
def get_shelf_ECCO_temperature_timeseries(project_folder):
    file_path = os.path.join(project_folder, 'Data', 'Modeling', 'ECCO', 'ECCOv5_THETA_Scoresby_Sund_Timeseries.nc')
    ds = nc4.Dataset(file_path)
...
def create_timeseries_plot(output_file, plot_titles, ecco_shelf_timeseries,
    ...
    ax1.plot(regional_L1_timeseries[:,0],regional_L1_timeseries[:,1],'-',color='olivedrab',label='L1 (RMSE: '+'{:.2f}'.format(0.5)+'$^{\circ}$C)')
    ax1.plot(regional_ecco_timeseries[:,0],regional_ecco_timeseries[:,1],'-',color='purple',label='L0 (RMSE: '+'{:.2f}'.format(0.6)+'$^{\circ}$C)')
    ...
    ax1.set_xticklabels([])
    # ax1.set_ylim([0, 1])
    ax1.set_xlim([1989.8, 2022.1])
    ax1.grid(linestyle='--', alpha=0.4, linewidth=0.5)
    ax1.set_title('Atlantic Water Temperature from Model Framework and Observations')
    ...
    ax2.plot(L1_temperature_timeseries[L1_locations.index('Sound Entrance')][:, 0],
             ...
             color='olivedrab',label='L1 (RMSE: '+'{:.2f}'.format(L1_RMSE[0])+'$^{\circ}$C)')
    ax2.plot(L2_temperature_timeseries[L2_locations.index('Sound Entrance')][:,0],
             ...
             label = 'L2 (RMSE: '+'{:.2f}'.format(L2_RMSE[0])+'$^{\circ}$C)',color='steelblue') #
    # ecco_shelf_timeseries[np.round(ecco_shelf_timeseries[:,0])==2018,1] = np.nan
    ...
    ax2.plot(omg_temperature_timeseries[omg_locations.index('Sound Entrance')][:, 0],
             omg_temperature_timeseries[omg_locations.index('Sound Entrance')][:, 1], 'k.', label='CTDs')
    ax2.plot(awi_temperature_timeseries[awi_locations.index('Sound Entrance')][:, 0],
             awi_temperature_timeseries[awi_locations.index('Sound Entrance')][:, 1], 'k.')#, label='OMG CTDs')
    ax2.set_ylabel('Sound Entrance\nTemperature ($^{\circ}$C)\n(200-450 m)')
    ax2.set_ylabel('b) Sound Entrance\nTemperature ($^{\circ}$C)\n(200-450 m)')
    ax2.legend(loc=2,ncol=2)
    ax2.set_xticklabels([])
    ax2.set_xlim([1989.8,2022.1])
    ...
    ax3.plot(L1_temperature_timeseries[L1_locations.index('Mid-Sound')][:, 0],
             ...
             label='L1 (RMSE: '+'{:.2f}'.format(L1_RMSE[1])+'$^{\circ}$C)',color='olivedrab') #
    ax3.plot(L2_temperature_timeseries[L2_locations.index('Mid-Sound')][:, 0],
             ...
             label='L2 (RMSE: '+'{:.2f}'.format(L2_RMSE[1])+'$^{\circ}$C)',color='steelblue') #
    ax3.plot(omg_temperature_timeseries[omg_locations.index('Mid-Sound')][:, 0],
             omg_temperature_timeseries[omg_locations.index('Mid-Sound')][:, 1], 'k.', label='CTDs')
    ax3.plot(awi_temperature_timeseries[awi_locations.index('Mid-Sound')][:, 0],
             awi_temperature_timeseries[awi_locations.index('Mid-Sound')][:, 1], 'k.')
    ax3.set_ylabel('c) Mid-Sound\nTemperature ($^{\circ}$C)\n(200-400 m)')
    ax3.set_xticklabels([])
    ax3.set_xlim([1989.8, 2022.1])
    ...
    ax4.plot(L2_temperature_timeseries[L2_locations.index('Near-DJG')][:, 0],
             ...
             label='L2 (RMSE: '+'{:.2f}'.format(L2_RMSE[2])+'$^{\circ}$C)',color='steelblue') #
    ax4.plot(omg_temperature_timeseries[omg_locations.index('Near-DJG')][:, 0],
             omg_temperature_timeseries[omg_locations.index('Near-DJG')][:, 1], 'k.', label='CTDs')
    ax4.plot(awi_temperature_timeseries[awi_locations.index('Near-DJG')][:, 0],
             awi_temperature_timeseries[awi_locations.index('Near-DJG')][:, 1], 'k.')
    ax4.set_ylabel('d) Near-DJG\nTemperature ($^{\circ}$C)\n(200-450 m)')
    ax4.set_xlim([1989.8, 2022.1])
    ax4.grid(linestyle='--', alpha=0.4, linewidth=0.5)
    ax4.legend(loc=2)

    if for_publication:
        output_file=output_file[:-3]+'pdf'
...
L0_output_names = ['','Sound Entrance']
L0_RMSE = calculate_RMSE([0,ecco_shelf_timeseries], omg_temperature_timeseries, L0_output_names)
print(L0_RMSE)

print('    - Reading in AWI timeseries')
awi_locations = plot_titles
AWI_locations = ['']+plot_titles[1:]
AWI_output_names = ['']+output_names[1:]
...
output_file = os.path.join(project_folder,'Figures','Ocean','Timeseries Comparison',
                                                            'Scoresby_Sund_Temperature_Comparison_With_Observations.png')
