...
def read_profiles_from_ECCO_files(ecco_dir,field_name, row, col, tile, RC):
    ...
    for year in years:
        ...
        ds.close()
        time[timestep_counter:timestep_counter+12] = year+np.arange(1/24, 1+1/24, 1/12)
        for t in range(12):
...
# location_name = 'Shelf_Entrance'
# location = (-21.339,70.085)
...
time, RC, timeseries = read_profiles_from_ECCO_files(ecco_dir,field_name, row, col, tile,RC)

output_file = os.path.join(project_dir,'Data','Modeling','ECCO','ECCOv5_'+field_name+'_'+location_name+'_profile_timeseries.nc')
