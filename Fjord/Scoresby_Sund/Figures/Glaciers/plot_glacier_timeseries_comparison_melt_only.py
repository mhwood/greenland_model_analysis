...
def plot_comparison_figure(output_file, retreat_timeseries,
                           ...
                           melt_timeseries, cumulative_melt_timeseries):
    ...
    plt.ylabel('a) Modeled \nIce Front Melt\n(m)')
    plt.gca().set_xticklabels([])
    plt.title('Daugaard-Jensen Modeled Melt')
    ...
    # plt.text(2005,np.max(cumulative_melt_timeseries[:,1]), 'Start of Ice Acceleration $\\rightarrow$ ', ha='right', va='top')
    # plt.text(2011, np.min(cumulative_melt_timeseries[:, 1]), ' $\\leftarrow$ Start of Ice Retreat\n       and Thinning', ha='left', va='bottom')
    ...
    plt.grid(linestyle='--', alpha=0.5)
    plt.ylabel('b) Modeled Integrated\nIce Front Melt Anomaly\n(m)')
    # plt.gca().set_xticklabels([])
    ...
    # plt.gca().set_xlim([1985, 2025])
    # plt.grid(linestyle='--', alpha=0.5)
    # plt.ylabel('e) Ice Front Position\n(km, relative to\n1985-2010 mean)')
    # plt.legend(loc=2, ncol=2)
...
output_file = os.path.join(project_folder,'Figures','Glacier','_'.join(glacier.split())+'_Dynamics_Timeseries_Melt_Only.png')
