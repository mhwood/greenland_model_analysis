...
def read_L1_timeseries_at_CTD_locations(config_dir, L1_model_name, locations, L1_XC, L1_YC):

    # find the indices of the locations in L1
    ...
    for ll in range(np.shape(locations)[0]):
        dist = great_circle_distance(locations[ll,0], locations[ll,1], L1_XC.ravel(), L1_YC.ravel())
        dist = np.reshape(dist,np.shape(L1_XC))
    ...
    output_dir = os.path.join(config_dir,'L1',L1_model_name,'results','state_3D_mon_mean')
    output_files = os.listdir(output_dir)

    # loop through the files and stack em into a big grid
    L1_timeseries_theta = np.zeros((np.shape(locations)[0], 30*12, Nr))
    L1_timeseries_salt = np.zeros((np.shape(locations)[0], 30 * 12, Nr))
    iterations = np.zeros((30*12,))
    counter = -1
                ...
                if file_name[0]!='.' and file_name[-3:]=='.nc':
                    if file_name.split('.')[1]==datestr:
                        ...
                        for ll in range(len(rows)):
                            L1_timeseries_theta[ll,counter,:] = Theta[0,:, rows[ll], cols[ll]]
                            L1_timeseries_salt[ll, counter, :] = Salt[0, :, rows[ll], cols[ll]]

    return(iterations, depths, L1_timeseries_theta, L1_timeseries_salt)


def store_timeseries_as_nc(project_dir, iterations, depths, L1_timeseries_theta, L1_timeseries_salt, L2_CTD_locations):
...
L1_XC, L1_YC = read_model_grid(config_dir, L1_model_name)
...
iterations, depths, L1_timeseries_theta, L1_timeseries_salt = \
    read_L1_timeseries_at_CTD_locations(config_dir, L1_model_name, L2_CTD_locations, L1_XC, L1_YC)


store_timeseries_as_nc(project_dir, iterations, depths, L1_timeseries_theta, L1_timeseries_salt, L2_CTD_locations)